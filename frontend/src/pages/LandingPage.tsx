...
export function LandingPage() {
  ...
  const resetConversion = () => { setConversionError(null); setConversionProgress(null); setPdfResult(null); };
  const createPdf = async () => {
    if (!images.length || isConverting) return;
    setIsConverting(true); setConversionError(null); setPdfResult(null);
    try {
  ...
  const handleDownload = () => {
    ...
    downloadPdf(pdfResult.blob, `pictapdf-${date}.pdf`);
    setStatusMessage('Your PDF download has started.');
  };

  return (
    <div id="top" className="site-page"><Navbar /><main>
    ...
    </main><footer className="footer"><div className="shell footer-inner"><a className="brand" href="#top"><span className="brand-mark"><span /></span><span>PictaPDF</span></a><p>Clear documents, minus the clutter.</p><div><a href="#privacy">Privacy</a><a href="#privacy">Terms</a><a href="#how-it-works">How it works</a></div></div></footer></div>
  );
}
